'use strict';
var confirmWithContract = function($scope, $sce, walletService) {
    ...
    $scope.confirmedOwners=function(){
        ...
        //操作id
        var opid = $scope.contract.functions[$scope.confirmFuncId].inputs[0].value;
    ...
    $scope.generateContractTx = function() {
        ...
        uiFuncs.getOwners(param,function(data){
            ...
            //判断是否已经确认过了
            if($scope.confirmAddresses!=null && $scope.confirmAddresses.length>0){
                if($scope.contains($scope.confirmAddresses,$scope.wallet.getAddressString())){
                    $scope.notifier.warning("用户已经确认过该交易");
                    return;
                }
            }
...
module.exports = confirmWithContract;
