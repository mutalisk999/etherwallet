'use strict';
var executeWithContract = function($scope, $sce, walletService) {
    ...
    $scope.generateTx = function() {
        try {
            ...
            //校验输入的金额
            else if(!globalFuncs.isFloat($scope.contract.functions[$scope.functionId].inputs[1].value)) throw globalFuncs.errorMsgs[0];
            //判断合约余额
            uiFuncs.queryBalance($scope.addressDrtv.ensAddressField,function(data){
                var amount = parseFloat($scope.contract.functions[$scope.functionId].inputs[1].value);
                var balance = 0;
                //console.log('查询合约余额：'+JSON.stringify(data));
                if(!data.isError){
                    balance = parseFloat(etherUnits.toEther(data.data.balance, 'wei'));
                    if(balance<amount){
                        $scope.sendContractModal.close();
                        $scope.notifier.danger("多签钱包合约余额不足");
                        return;
                    }
    ...
    $scope.saveTransaction=function(txId){

        var param={
            txId:txId,
            from:$scope.wallet.getAddressString(),
            txTime:uiFuncs.getCurrentTime(),
            contractAddress:$scope.addressDrtv.ensAddressField,
            to:$scope.contract.functions[$scope.functionId].inputs[0].value,
            txAmount:parseFloat($scope.contract.functions[$scope.functionId].inputs[1].value),
            fromChain:$scope.ajaxReq.type
        };
        $scope.local.saveTx(param,function(data){
            //console.log("本地保存交易返回"+data);
            });
    }
    ...
    $scope.sendTx = function() {
        ...
        uiFuncs.sendTx($scope.signedTx, function(resp) {
            //console.log("交易返回信息："+JSON.stringify(resp));
            /*resp={
                isError:false,
                data:'0xe670ec64341771606e55d6b4ca35a1a6b75ee3d5145a99d05921026d152733c3'
            };*/
            if (!resp.isError) {
                ...
                //弹框显示交易id
                $scope.alertTitle="交易id";
                $scope.alertContent=resp.data;
                $scope.alertModal.open();
                //保存交易信息
                $scope.saveTransaction(resp.data);
            } else {
    ...
    $scope.selectFunc = function() {
        ...
        for(var i=0;i<$scope.contract.functions.length;i++){
            if($scope.contract.functions[i].name=='execute'){
        ...
        for(var j=0;j<$scope.contract.functions[index].inputs.length;j++){
            if($scope.contract.functions[index].inputs[j].type=='address'){
                $scope.contract.functions[index].inputs[j].desc='转账目标地址';
            }
            if($scope.contract.functions[index].inputs[j].type=='uint256'){
                $scope.contract.functions[index].inputs[j].desc='转账金额(单位：'+ $scope.ajaxReq.type +")";
            }
            if($scope.contract.functions[index].inputs[j].type=='bytes'){
                $scope.contract.functions[index].inputs[j].desc='备注，用于区别同一个块中的不同交易';
            }
        }
        ...
        for (var i in curFunc.inputs) {
            if (curFunc.inputs[i].value) {
                if (curFunc.inputs[i].type.indexOf('[') !== -1 && curFunc.inputs[i].type.indexOf(']') !== -1) {
                    ...
                    //第二个参数特殊处理
                    if(i==1){
                        values.push(etherUnits.toWei(curFunc.inputs[i].value, "ether"));
                    }else{
    ...
    $scope.initContract = function() {
        try {
            ...
            $scope.selectFunc();
            $scope.tx.to=$scope.addressDrtv.ensAddressField;
        } catch (e) {
            $scope.notifier.danger(e);
        }
    }
...
module.exports = executeWithContract;
